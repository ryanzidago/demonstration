...
RUN apt-get update -y && apt-get install -y libstdc++6 openssl libncurses5 locales nodejs npm \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/demonstration ./
